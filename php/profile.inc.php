...
switch ($_GET['action']) {
    ...
    case 'updateMail':
        ...
        if (!filter_var($mail, FILTER_VALIDATE_EMAIL)) {
            ...
            $taskBoard->mysqliQueryPrepared("UPDATE users SET userMail = ?, userMailStatus = 'unverified' WHERE userID = ?", $mail, $userID);
            $taskBoard->sendVerifyMail($userID, $taskBoard->getMailByUserID($userID));
    ...
    case 'verifyMail':
        ...
        if ($tokenData) {
            ...
            $taskBoard->mysqliQueryPrepared("UPDATE users SET userMailStatus = 'verified' WHERE userID = ?", $userID);
